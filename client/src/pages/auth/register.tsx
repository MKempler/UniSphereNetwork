...
import PostRegistrationDidModal from '@/components/auth/PostRegistrationDidModal';
...
export default function Register() {
  ...
  const [newlyRegisteredUserDid, setNewlyRegisteredUserDid] = useState<string | null>(null);
  const [newlyRegisteredUsername, setNewlyRegisteredUsername] = useState<string | null>(null);
  ...
  const registerMutation = useMutation({
    mutationFn: async (userData: any) => {
      return apiRequest<any>("POST", "/api/auth/register", userData);
    },
    onSuccess: (response: any) => {
      if (response && response.user && response.sessionId) {
        setUserLanguage(form.getValues().language);
        setAuthState(response.user, response.sessionId);

        setNewlyRegisteredUserDid(response.user.did);
        setNewlyRegisteredUsername(response.user.username);
        setShowDidModal(true);
      } else {
        toast({
          title: "Registration Successful - Login Required",
  ...
  return (
    <>
      ...
      {newlyRegisteredUserDid && newlyRegisteredUsername && (
        <PostRegistrationDidModal 
          isOpen={showDidModal}
          onClose={() => setShowDidModal(false)} 
          userDid={newlyRegisteredUserDid}
